...
class EEGGraphConvNet(nn.Module):
    def __init__(self, n_channels, hidden_dim, n_layers=2, dropout=0.2):
        ...
        self.register_buffer(
            "adj_matrix",
            torch.FloatTensor(normalized_adj),
        )
        ...
        for i in range(n_layers):
            if i == 0:
                self.gcn_layers.append(
                    BatchGraphConvLayer(1, hidden_dim)
                )
            else:
                self.gcn_layers.append(
                    BatchGraphConvLayer(hidden_dim, hidden_dim)
                )
    ...
    def forward(self, x):
        ...
        node_feat = x_transposed.reshape(
...
class SpatialAttentionModule(nn.Module):
    def __init__(self, input_dim, n_channels=32, reduction_ratio=4):
        ...
        self.feature_transform = nn.Conv1d(
            input_dim,
            input_dim,
            1,
        )
    ...
    def forward(self, x):
        ...
        spatial_att_expanded = spatial_att.unsqueeze(2)
...
class SpatialFeatureExtractor(nn.Module):
    ...
    def forward(self, x):
        ...
        x_proj = self.output_proj(x_attention)
